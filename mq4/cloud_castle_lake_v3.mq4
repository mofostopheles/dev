...
//using indicators very sparingly, or should i say subltly.
//i've found that using them too strongly filters out some great moves
//so i'm only using them to very carely filter out noise at the very edges
//it almost amounts to nothing.

//the real trick is human judgement. very hard to code.
//we need to find proper levels, short and long. 
//when candles break those levels, then we've got a good candidate.
//indicators can tell us that only part of the time. 
...
void OnTick()
{
   ...
   if(
      ...
      //&&
      //iMA(_sym,0,34,0,MODE_EMA,PRICE_WEIGHTED,1) > iMA(_sym,0,34,0,MODE_EMA,PRICE_WEIGHTED,5)
      )
   {     
      //open new order
      ...
      if (  
            ...
            //&& //breakout candle isn't actually another hammer
