...
//***************** END VSI ENGINE ******************//
            ...
            int MM=TimeMinute(Time[i+1]);   // Minute
		      ...
		      {				      
		          ...
		          if ( iMA(Sym, TimeFrame, 2, TimeFrame, MODE_EMA, PRICE_HIGH, i+_lookback ) > iMA(Sym, TimeFrame, 12, TimeFrame, MODE_EMA, PRICE_TYPICAL, i+_lookback ))
		          {
		             _signalMatrix = StringConcatenate( _signalMatrix, "\"above\",");
		          }    	
		          ...
		          if ( minus12CandleHigh > iMA(Sym, TimeFrame, 34, TimeFrame, MODE_EMA, PRICE_LOW, i+_lookback ))
		          {
		             _signalMatrix = StringConcatenate( _signalMatrix, "\"above\",");
		          }   
