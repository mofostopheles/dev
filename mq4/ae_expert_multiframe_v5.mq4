//buy candle after LL IN AN UPTREND

//globally adjust tp/sl
double _stopLossFactor = 8;
double _takeProfitFactor = 6; //dynamically reset depending on the MACD
...
//***************** END LOOKBACK PATTERN SETUP ******************//
      ...
      int targetLockExpire = 3600; //in seconds
      ...
      */
      ...
      /*
      if (  
               Time[i+1] - _targetAcquisitionTime >= 1000
               //&& Time[i+1] - _targetAcquisitionTime <= targetLockExpire
               //minus1CandleClose <= _targetAcquisitionPrice-2*(10*_point)
               //&& minus1CandleLow >= _targetAcquisitionPrice-8*(10*_point) //but not too low
               && average143421MACD > 0
               && _WEAPON_ARMED == true
      ...
      /*------- FIRE --------*/
      if ( 
            StringFind(_candleLookback, "hh", 0) > -1
            && average143421MACD > 0
            && macdAverage > 0
            //_WEAPON_ARMED == true
            //&& _LOCKED_ON == true
