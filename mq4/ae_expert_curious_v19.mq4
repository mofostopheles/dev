//arlo emerson, 9/5/2012

//WHAT WE HAVE LEARNED

//this code, as is, finds numerous good entries.
//it also finds a bunch of dogs.
//win/loss ratio is approx 50/50
//this is the best i've coded so far
//whereas the first profitable expert i wrote (v.12) gets around 44/55.
//it trades less, but also misses some opps.

//we have proven we can find entries.
//we have proven we can filter out some noise.
//when we trade with the trend our odds increase dramatically.
//when we do a couple of weird tricks our odds increase.
...
/* ------------------------- TRADE LOGIC ---------------------------*/
              ...
              || (fiveMinMacdMinus1Candle > 0.0 && fiveMinMacdMinus1Candle < 0.000020))																						
				...
				{
					_lastZCrossingHigh = _thisZCrossingHigh; 
					_lastZCrossingLow = _thisZCrossingLow;
					_lastZCrossingClose = _thisZCrossingClose; 
					
					_thisZCrossingHigh = minus1CandleHigh;
					_thisZCrossingLow = minus1CandleLow;
					_thisZCrossingClose = minus1CandleClose;
					
					_lastZCrossingTime = _thisZCrossingTime;
					_thisZCrossingTime = Time[i+1];
										         	
         		if(						
				        _thisZCrossingClose > _lastZCrossingClose
				        && _thisZCrossingLow > _lastZCrossingLow
				        && _thisZCrossingHigh > _lastZCrossingHigh				        
				     )
				     {
					     ObjectCreate("targetConfirmedbuy"+_labelIndex,OBJ_TEXT,0,Time[i+1],minus1CandleLow);
					     ObjectSetText("targetConfirmedbuy"+_labelIndex,"Z",12,"Arial",Red);
				...
				/*
				{
      			...
      			_lastZCrossing = minus1CandleClose;
      			
      			ObjectCreate("targetAcquiredbuy"+_labelIndex,OBJ_TEXT,0,Time[i+1],minus1CandleLow);
         		ObjectSetText("targetAcquiredbuy"+_labelIndex,"Z",12,"Arial",Yellow);
         		//Print("next res: ", _resistencePrices[0]);
				}
				*/
