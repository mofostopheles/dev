         	...
         	if (
         	     ...
         	     )
         	     {
                    ...
                    //latest price is at 0, 
							...
							//Print("_resistencePrices[_resistencePriceCounter]   ", _resistencePrices[0]);                               	     
         	     }
...
bool imminentResistence(double priceToTest)
{
   ...
   //Print("^^^^^we are here ^^^^^^", ( _resistencePrices[0] - priceToTest ) );
...
bool resistenceBroken(double priceToTest)
{
   ...
   for(int k=0;k<tmpArraySize;k++)
   {
   	//filter out prices we've already passed through
   	
   
      if ( //if price is within 10 pips
      		priceToTest > _resistencePrices[k])
      {
