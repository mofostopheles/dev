...
int init()
{
   ...
   string vsiTitle = "volume and price - imminent swing finder --- v8";
   Sym = Symbol();
...
//***************** END LOOKBACK PATTERN SETUP ******************//
      ...
      //patterns do not need to be symetrical for long and short 
      //find patterns that work for each direction and mark them with wins/losses
      ...
      if (
         ...
         _signalPattern == "xll|x|x" //0/148
         
         
         //_signalPattern == "x|downll|down"
         //_signalPattern == "uphh|x|downll"
         
         //_signalPattern == "hh|xll|x"
         //_signalPattern == "xhh|downll|x"
         //_signalPattern == "ll|down|down"
         //_signalPattern == "xhh|down|down"
         //_signalPattern == "up|up|x" 
         //_signalPattern == "asdf" 
         )
      {
            if (_inTrade==false)
            ...
            /*
            else if (_inTrade==true && _tradeDirection == "buy")//existing trade is a buy
            {            
               //determine if the order we just closed was profitable
               reconcileOrder(high, low, open, close);               
               
               //back out of current trade, take new signal
      ...
      if(
         //best patterns first
         _signalPattern == "xhh|x|x" //wow!!! 385/149  (12/50 sl/tp)       
         //_signalPattern == "x|x|uphh" //239/1
         
         //_signalPattern == "sadf"
         //_signalPattern == "x|uphh|up" //666/568
         //_signalPattern == "downll|x|uphh" //242/244
         //_signalPattern == "ll|xhh|x" //38/12
         //_signalPattern == "xll|uphh|x" //90/89
         //_signalPattern == "hh|up|up" //71/53
         //_signalPattern == "xll|up|up" //6/88
         //_signalPattern == "down|down|x" //457/552
         ...
         )
      {
            ...
            else if (_inTrade==true && _tradeDirection == "sell")//existing trade is a sell         
            {
...
//            Alert("loser 3");
               _losers++;
               //Alert( StringConcatenate("exited at   ", high, " ", low, " ", open, " ", close, " ||| tp: ", _takeProfit, "  sl: ", _stopLoss));
               _inTrade = false;
               _tradeDirection = "";
...
int reconcileOrder(double pHigh, double pLow, double pOpen, double pClose)
{
         if(_tradeDirection == "buy")
         {
            if (
         ...
         else if(_tradeDirection == "sell")
         {
               ...
               )
            {
              ...
              //Alert( StringConcatenate("exited at   ", pHigh, " ", pLow, " ", pOpen, " ", pClose, " ", _takeProfit, "   ", _stopLoss));
               Alert("loser 1");
               _losers++;
            }
            else if(
