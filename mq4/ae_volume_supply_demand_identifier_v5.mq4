/*
   ...
   - refactored to redraw the entire chart label and signals at the beginning of each candle
   - todo: isolate signals, write orders
   ...
   - open another chart running v4 to check for signal accuracy
...
int init()
{
   ...
   string vsiTitle = "volume and price - imminent trend finder --- v5";
   Sym = Symbol();
...
int start()
{
	...
	if (_lastAlertTime < Time[0] || _firstRun == false) //a candle just opened
   {
      ...
      _labelIndex = 0;
         ...
         for(int i = 0; i < Bars; i++) //repaint all the bars
         {
...
//***************** END HH/LL DATA CAPTURE ******************//		       
     ...
     else if(StringFind(_minus1PriceAttributes, "ll|", 0) > -1)//lower low
     {
      ...
      //shuffle the _lookbackSignalPattern so that the latest candle is at position 0
      _lookbackSignalPattern[2] = _lookbackSignalPattern[1];
      _lookbackSignalPattern[1] = _lookbackSignalPattern[0];
      _lookbackSignalPattern[0] = _candleLookback;
