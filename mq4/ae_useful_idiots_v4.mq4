//arlo emerson, 3/28/2013

/*
   tuned for USDJPY 15m charts, it will not do anything on other pairs because of the standard dev settings.
	holy shit this is really profitable.
	right now it's set up for buys only.
	this algo has more losers than winners but the with 110TP/21SL it's profitable.
	crazy. i've never created an algo this good before.
*/

//globally adjust tp/sl
double _stopLossFactor = 21;
double _takeProfitFactor = 110;
...
/* ------------------------- TRADE LOGIC ---------------------------*/
		...
		if (
					...
					&& iStdDev(NULL, TimeFrame, 7, 0, MODE_SMA, PRICE_CLOSE, i+1) > 0.1
					&& minus1CandleHigh - minus1CandleLow < 30*(10*_point)
					&& minus1CandleHigh - minus1CandleLow > 20*(10*_point)
					&& minus1CandleHigh > ema34HighMinus1Candle
			)
			{
				_targetPrice = ( minus1CandleOpen - 10*(10*_point) );
			}
			
				
		//BUYING PATTERN MATCH
		if (
				minus1CandleLow < _targetPrice
            ...
            if (OrdersTotal() > 0)
            {
               ObjectCreate("buy"+_labelIndex,OBJ_ARROW,0,Time[i+1],_ask);
               ObjectSet("buy"+_labelIndex, OBJPROP_ARROWCODE, SYMBOL_LEFTPRICE);
               ObjectSet("buy"+_labelIndex,OBJPROP_COLOR,MediumBlue);
            }
				...
				{   
                  ...
                  ObjectCreate("buy"+_labelIndex,OBJ_ARROW,0,Time[i+1],_ask);
                  ObjectSet("buy"+_labelIndex, OBJPROP_ARROWCODE, SYMBOL_LEFTPRICE);
                  ObjectSet("buy"+_labelIndex,OBJPROP_COLOR,Yellow);
