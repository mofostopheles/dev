...
//***************** BEGIN PATTERN ******************// 
	...
	{
		//start the line
		_volumeLineHolder[0] = Time[i+1];
		_volumePriceHolder[0] = minus1CandleClose;
		_volumeLineCounter++;	
		_lineEnded = false;	
	}
   ...
   if (  
	...
	{
		//continue the line
		_volumeLineHolder[_volumeLineCounter] = Time[i+1];
		_volumePriceHolder[_volumeLineCounter] = minus1CandleClose;
		_volumeLineCounter++;	
		_lineEnded = false;	
	}
	...
	if (_volumeLineCounter > 1 && _lineEnded == true)
	{	
		ObjectCreate("buysellSRLine" + _labelIndex,OBJ_TREND,0,_volumeLineHolder[0], _volumePriceHolder[0] ,_volumeLineHolder[_volumeLineCounter], _volumePriceHolder[_volumeLineCounter] );
      ...
      //if the close was above a Standard Deviance (7 period) of .0003
		//set the halfway point of the line to be an entry line
		//that point will be retested. 
		//if the test holds the trend is real. if not, your SL gets hit.
		if (  iStdDev(NULL, 5, 7, 0, MODE_SMA, PRICE_CLOSE, i+1) >= 0.0003 )
		{

         double tmpMidwayPrice = (_volumePriceHolder[0] + _volumePriceHolder[_volumeLineCounter])/2;
         
         if (_volumePriceHolder[_volumeLineCounter] - _volumePriceHolder[0] > 0)
         {
            _trendDirection = "up";
         }
         else
         ...
         {         
            ObjectCreate("buy"+_labelIndex,OBJ_ARROW,0,Time[i],tmpMidwayPrice);
            ObjectSet("buy"+_labelIndex, OBJPROP_ARROWCODE, SYMBOL_LEFTPRICE);
            ObjectSet("buy"+_labelIndex,OBJPROP_COLOR,Green); 
         }
         else if ( minus1CandleClose < minus1_34EmaClose)
         { 
            ObjectCreate("sell"+_labelIndex,OBJ_ARROW,0,Time[i],tmpMidwayPrice);
            ObjectSet("sell"+_labelIndex, OBJPROP_ARROWCODE, SYMBOL_LEFTPRICE);
            ObjectSet("sell"+_labelIndex,OBJPROP_COLOR,Red); 
         }
         */
         /*
            ObjectCreate("buysellHorzLine" + i+1,OBJ_TREND,0, Time[i+1], tmpMidwayPrice, Time[i-10], tmpMidwayPrice );
            ObjectSet("buysellHorzLine" + i+1,OBJPROP_COLOR,DarkGray);
            ObjectSet("buysellHorzLine" + i+1,OBJPROP_STYLE,STYLE_DOT);
            ObjectSet("buysellHorzLine" + i+1,OBJPROP_WIDTH,1);
            ObjectSet("buysellHorzLine" + i+1, OBJPROP_RAY,false);
         */
		    /*
		    ObjectCreate("buysellTestLine" + _labelIndex,OBJ_TREND,0,_volumeLineHolder[0], _volumePriceHolder[0] ,_volumeLineHolder[_volumeLineCounter], _volumePriceHolder[_volumeLineCounter] );
		    ObjectSet("buysellTestLine" + _labelIndex,OBJPROP_COLOR,Orange);		    
		    ObjectSet("buysellTestLine" + _labelIndex,OBJPROP_WIDTH,1);
		    ObjectSet("buysellTestLine" + _labelIndex,OBJPROP_RAY,false);
		    */
		}
		
		
		_volumeLineCounter=0;
		
      ArrayResize(_volumeLineHolder, 99);
      ArrayResize(_volumePriceHolder, 99);
      
		
	}
	
	if (_trendDirection == "down")
	{
	     //sell next upper boll line break
	     if (minus1CandleHigh > iBands(Sym,0,12,2.23,-3,PRICE_HIGH,MODE_UPPER,i+1))
	     {
	...
	else if (_trendDirection == "up")
	{
	     //buy next lower boll break
	     if (minus1CandleLow < iBands(Sym,0,12,2.23,-3,PRICE_LOW,MODE_LOWER,i+1))
	     {
