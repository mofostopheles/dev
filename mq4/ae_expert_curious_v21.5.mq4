...
/* ------------------------- IDENTIFY LEFT WALL OF DAM ---------------------------*/
int tmpLastShift = iBarShift(Sym, PERIOD_M5, TimeCurrent(), true);


//if the candle following acquisition is higher, invalidate the acquisition
/*
   if (
         _TARGET_ACQUIRED == true
         && (TimeCurrent() - _targetAcquisitionTime) == 300
         && minus1CandleHigh > _targetAcquisitionHigh
      )
      {
         _TARGET_ACQUIRED = false;	 
      }
*/
//find potential targets...construction of left wall      

if (
      lastHighInOpenAir(  minus1CandleHigh, 21, tmpLastShift ) == true
      && isRanging(minus1CandleOpen, minus1CandleClose, minus1CandleHigh, minus1CandleLow) == false
      //&& _TARGET_ACQUIRED == false
      //always the latest target
   )
   {
      ObjectCreate("buyDamLeftWall"+_labelIndex,OBJ_TEXT,0,Time[i+1],minus1CandleLow);
		ObjectSetText("buyDamLeftWall"+_labelIndex,"#1",9,"Arial",Red);		
...
//if support doesn't form, invalidate  
/*
if (
      _TARGET_ACQUIRED == true
      && Time[i+1] > _targetAcquisitionTime   
      && minus1CandleLow < (_targetAcquisitionLow - (2*(10*Point)))
      && minus1CandleHigh > _targetAcquisitionHigh
   )
   {
      _TARGET_ACQUIRED = false;
...
if (
      ...
      && minus1CandleOpen < _targetAcquisitionHigh  
      ...
      && supportFormedBetween(_targetAcquisitionTime, TimeCurrent()) == true
   )
{
   _TARGET_LOCKED = true;
   ObjectCreate("buyDamRightWall"+_labelIndex,OBJ_TEXT,0,Time[i+1],minus1CandleLow);
	ObjectSetText("buyDamRightWall"+_labelIndex,"#2",9,"Arial",YellowGreen);	
	//draw a line connecting the left and right wall
}	
...
bool supportFormedBetween(int timeStart, int timeEnd)
{
   int tmpShiftStart = iBarShift(Sym, PERIOD_M5, timeStart, true);
   int tmpShiftEnd = iBarShift(Sym, PERIOD_M5, timeEnd-300, true);

   int tmpShift = iHighest(Sym, PERIOD_M5, MODE_HIGH, (tmpShiftStart-(tmpShiftEnd-1)), (tmpShiftEnd-1));
   double tmpPriceOfHH = High[tmpShift];
   /////
   tmpShift = iLowest(Sym, PERIOD_M5, MODE_LOW, (tmpShiftStart-(tmpShiftEnd-1)), (tmpShiftEnd-1));
   double tmpPriceOfLL = Low[tmpShift];
   

   if (
         tmpPriceOfHH < _targetAcquisitionHigh
         && tmpPriceOfLL > _targetAcquisitionLow
       )
   {
      return (false);
   }
   ...
   ObjectCreate("waterlinebuysell" + _labelIndex,OBJ_TREND,0, timeStart, _targetAcquisitionHigh, timeEnd-300, _targetAcquisitionHigh );
   ObjectSet("waterlinebuysell" + _labelIndex,OBJPROP_COLOR,Magenta);
   ObjectSet("waterlinebuysell" + _labelIndex,OBJPROP_WIDTH,1);
   ObjectSet("waterlinebuysell" + _labelIndex,OBJPROP_RAY,false); 
...
bool lastHighInOpenAir(double priceToTest, int pLookback, int pIndex)
{
   int counter = 0;
   //start at 2 because we are always passing in the +1 candle
   for (int z=2;z<=pLookback;z++)
   {  
      ...
      {      
        //ObjectCreate("highbuy"+_labelIndex,OBJ_TEXT,0,Time[z+pIndex],priceToTest);
        // ObjectSetText("highbuy"+_labelIndex,"x",14,"Arial",Orange); 
