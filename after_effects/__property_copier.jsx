...
DESCRIPTION
    ...
    a script that copies stuff from one comp into many comps
        --> set the master comp to copy from (select from dropdown menu)
        --> select a bunch of comps
        --> loop the layers of the master and copy position/keyframes of each layer
        --> 
...
var LABEL_COMP_MASTER_SOURCE_NAME = "Select a master comp to copy from:";
var LABEL_MASTER_LAYER_NAME = "Select a layer to copy properties from:";
var LABEL_SWAP_BUTTON = "Swap layer with comp";
var LABEL_TRANSFER_PROPS_BUTTON = "Transfer";
var LABEL_PERFORM_COPY_BUTTON = "Copy Layer Props";
var MARGIN_LEFT = 50;
...
function buildUI()
{
    if (win != null)
    {
        ...
        var strMasterComp = 
        "panel { orientation:'row', text:'Select a master comp and a layer', spacing:5, margins:10, \
            leftCol: Group { orientation:'column', alignment:['fill','top'], \
                ...
                botRow: Group { alignment:['left','top'], \
                    lblLayerToCopyFrom: StaticText { text:'" + LABEL_MASTER_LAYER_NAME + "', alignment:['left','center'] }, \
                    ddlLayerToCopy: DropDownList { text:'', characters:30, alignment:['fill','center'], preferredSize:[250, *] }, \
                }, \
            } \
        }";
        
        win.objMasterComp = win.add(strMasterComp);
        //win.objMasterComp.rightCol.swapButton.onClick = function() { alert("not implemented"); swapItems( win.objMasterComp ) };
        win.objMasterComp.leftCol.filterRow.searchButton.onClick = function() { populateSearchResults( win.objMasterComp.leftCol.topRow.ddlMasterComp, win.objMasterComp.leftCol.filterRow.txtMasterCompFilter.text ) };
        win.objMasterComp.leftCol.topRow.ddlMasterComp.onChange = function() { populatePrimaryLayerList( win.objMasterComp.leftCol.topRow.ddlMasterComp.selection.text, win.objMasterComp.leftCol.botRow.ddlLayerToCopy ) };
        win.objMasterComp.leftCol.botRow.ddlLayerToCopy.onChange = function() { populateTargetLayerList( win.objGroup2.leftCol.topRow.ddlTargetLayer ) };


        //------------------------- TARGET LAYER SECTION -------------------------

        var strTargetLayer = 
        "panel { orientation:'row', text:'Select a target layer', spacing:5, margins:10, alignment:['fill','top'], \
            leftCol: Group { orientation:'column', alignment:['fill','top'], \
                topRow: Group { alignment:['fill','top'], \
                    lblTargetLayer: StaticText { text:'Choose a target layer', alignment:['left','center'] }, \
                    ddlTargetLayer: DropDownList { text:'', characters:30, alignment:['fill','center']  },  \
                }, \
                botRow: Group { alignment:['fill','top'], \
                    lblLayerToCopyFrom: StaticText { text:'Select properties to transfer: ', alignment:['left','center'] }, \
                    cbxAnchorPoint: Checkbox { text:'anchor point', alignment:['fill','center'], value:true }, \
                    cbxPosition: Checkbox { text:'position', alignment:['fill','center'], value:true }, \
                    cbxScale: Checkbox { text:'scale', alignment:['fill','center'], value:true }, \
                }, \
                botRow2: Group { alignment:['fill','top'], \
                    cbxRotation: Checkbox { text:'rotation', alignment:['fill','center'], value:true }, \
                    cbxOpacity: Checkbox { text:'opacity', alignment:['fill','center'], value:true }, \
                    cbxFX: Checkbox { text:'fx', alignment:['fill','center'], value:true }, \
                    cbxTime: Checkbox { text:'time', alignment:['fill','center'], value:true }, \
                }, \
            } \
        ...
        win.objGroup2 = win.add(strTargetLayer);
        populateTargetLayerList(win.objGroup2.leftCol.topRow.ddlTargetLayer);
        //populateCompList(win.objGroup2.leftCol.botRow.ddlLayerToCopy, SEARCH_STRING_UI);
        win.objGroup2.rightCol.btnDoAction.onClick = function() { transferProperties() };
        ...
        win.objGlobalButtons.cmds.refreshLayersButton.onClick = function() { populatePrimaryLayerList(win.objMasterComp.leftCol.topRow.ddlMasterComp);populatePrimaryLayerList(win.objGroup2.leftCol.topRow.ddlMasterComp); };
...
//loop the layers of the first selected comp and use that as our list of layers to choose from
function populatePrimaryLayerList(pSelectedCompName, pList)
{
    pList.removeAll();

    var tmpComp = getComp(pSelectedCompName);

    for (var j=1; j<=tmpComp.layers.length; j++)
    {
        pList.add('item', tmpComp.layers[j].name);                        
    }
}
...
{
    app.beginUndoGroup("transferProperties");

    if ( null == win.objGroup2.leftCol.topRow.ddlTargetLayer.selection )
    {
        alert("target layer is null");
        return;
    }

    var layerNameToCopyFrom = win.objMasterComp.leftCol.botRow.ddlLayerToCopy.selection.text;
    var compNameToCopyFrom = win.objMasterComp.leftCol.topRow.ddlMasterComp.selection.text;
    var layerNameToCopyTo = win.objGroup2.leftCol.topRow.ddlTargetLayer.selection.text;
    var masterComp = getComp(compNameToCopyFrom);
    var masterLayer = null;

    if (null == masterComp){alert("master comp is null.");}

    for (var j=1; j<=masterComp.layers.length; j++)
    {
        if (masterComp.layers[j].source.name == layerNameToCopyFrom)
        {
            masterLayer = masterComp.layers[j];
            break;
        }    
    }

    alert("updating layer '" + layerNameToCopyTo + "'' from layer '" + layerNameToCopyFrom + "'' from comp '" + compNameToCopyFrom + "'");
        ...
        else 
        {
            ...
            if (selectedComps >= 1)
            {
                for (var k=0; k<collectionSelectedComps.length; k++)
                {
                    ...
                    /*
                    if (layerNameToCopyFrom == "")
                    {
                        strError = "Please select a valid layer.";
                    }
                    ...
                    */
                    ...
                    {
                        if (selectedComp.layers[j].source.name.indexOf( layerNameToCopyTo ) != -1 )
                        {
                            //selectedComp.layers[j].replaceSource(replacementComp,false);
                            //selectedComp.layers[j].name = replacementComp.name;

                            cycleProperties(masterLayer, selectedComp.layers[j]);
                    ...
                    {
                        alert("couldn't find a layer in " + selectedComp.name + " with name '" + layerNameToCopyTo + "'");
                        selectedComp.label = 1;
                    }
                    else
                    {
                        statusReport += "\n\n" + selectedComp.name + " - replaced '" + layerNameToCopyTo + "'' with props from '" + compNameToCopyFrom + " " + layerNameToCopyFrom + "'"; 
                    }                    
                }
            } 
            else 
...
function cycleProperties(pSourceLayer, pTargetLayer)
{
    if (win.objGroup2.leftCol.botRow.cbxAnchorPoint.value == true)
    {
        alert(pSourceLayer);
        pSourceLayer.name = asdf;
        //pSourceLayer("position").value
        //pSourceLayer("position").numKeys
        
    }
    
    if (win.objGroup2.leftCol.botRow.cbxPosition.value == true) {}
    if (win.objGroup2.leftCol.botRow.cbxScale.value == true) {}
    if (win.objGroup2.leftCol.botRow2.cbxRotation.value == true) {}
    if (win.objGroup2.leftCol.botRow2.cbxOpacity.value == true) {}
    if (win.objGroup2.leftCol.botRow2.cbxFX.value == true) {}
}
