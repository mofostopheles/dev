 ...
 * Add flares above each "connection" null. Flare is taken from FLARE_MASTER layer.
 */
var flareInsertAndSetStart = function() {
    return {
            ...
            for (var k = this.arrSelectedComps.length - 1; k >= 0; k--) {
                ...
                for (var j=tmpSelectedLayers.length-1; j>=0; j--) {
                    if (tmpSelectedLayers[j].name.indexOf('connection') != -1) {
                        ...
                        // snarf out the frame start from the connection name
                        // note, this will only be right some of the time,
                        // because the sweeps sometimes are run in reverse.
                        var startString = tmpSelectedLayers[j].name.indexOf(":");
                        var endString = tmpSelectedLayers[j].name.indexOf("-");
                        var frameInPoint = tmpSelectedLayers[j].name.substring(startString+2, endString);
                        var timeInPoint = currentFormatToTime(frameInPoint, 30);

                        tmpSelectedLayers[j].startTime = timeInPoint;
                        tmpLayer.startTime = timeInPoint;
 ...
 */
flareInsertAndSetStart().main(vars.layerToFind);
