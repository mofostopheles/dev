...
'''
    ...
    v2, experiment with some crude animation to simulate earthquake
...
class TaipeiBuilding():
    ...
    def __init__(self):
        print('constructor')
        # the following variables are to store coords of
        # a location in the building where we can
        # later use as a starting point for the animation
        self.storedX = 0
        self.storedY = 0
        self.storedZ = 0
    ...
    def generateStructure(self):
        ...
        for i in range(0, 16, 2):
            ...
            if i == 10:
                # arbitrary place to store coords
                # these will be used as a starting point for the animation
                self.storedX = newX
                self.storedY = newY + i
                self.storedZ = newZ
        ...
        w.player.pos = Vector(newX + 5, newY + 5, newZ - 3)
        

    def repoToAltitude(self):
        w.player.pos = Vector(w.player.pos.x, 40, w.player.pos.y)
    
    # loop through the blocks in the building
    # floor-by-floor and shift them
    # see if the resulting animation is acceptable
...
tb.generateStructure()
tb.simulateEarthquake()
#tb.repoToAltitude()
print("all done!")
