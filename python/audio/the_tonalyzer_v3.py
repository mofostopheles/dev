...
sourceFileName = 'sky.wav'
targetFileName = 'sky.wav'
...
for i in range(len(targetListOfFrequencies)):
	for y in range (len(listOfFrequencies)):
		#if round( targetListOfFrequencies[i], roundingAmount) > round( listOfFrequencies[y], roundingAmount )-.5 and round( targetListOfFrequencies[i], roundingAmount) < round( listOfFrequencies[y], roundingAmount )+.5:
		if round( targetListOfFrequencies[i], roundingAmount) > round( listOfFrequencies[y], roundingAmount )-.5 and round( targetListOfFrequencies[i], roundingAmount) < round( listOfFrequencies[y], roundingAmount )+.5:
...
tmpName = targetFileName + "_" + str(datetime.datetime.now().microsecond) + "_xx_borked_" + str(sampleRange) + ".wav"
print tmpName
